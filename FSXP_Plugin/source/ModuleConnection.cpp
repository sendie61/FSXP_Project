...
void ModuleConnection::OnAccept(const std::string & host, uint16_t port) {
	PPL::Log() << PPL::Log::Info << "[ModuleConnection::" << __FUNCTION__ << "] " << "Connection with "<< host << " on port:" << port << " Accepted." << PPL::Log::endl;
...
void ModuleConnection::OnConnect(const std::string & host, uint16_t port) {
	PPL::Log() << PPL::Log::Info << "[ModuleConnection::" << __FUNCTION__ << "] " << "Connected with "<< host << " on port:" << port << PPL::Log::endl;
...
void ModuleConnection::OnError(const boost::system::error_code & error) {
	PPL::Log() << PPL::Log::Info << "[ModuleConnection::" << __FUNCTION__ << "] " << error.message() << PPL::Log::endl;}
