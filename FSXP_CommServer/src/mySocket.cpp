...
mySocket::mySocket() :
		portNumber(0), socketId(0), blocking(0), bindFlag(0) {
}

mySocket::mySocket(int pNumber) :
		portNumber(pNumber), socketId(0), blocking(1), bindFlag(0) {
...
void mySocket::setDebug(int debugToggle) {
	try {
		if (setsockopt(socketId, SOL_SOCKET, SO_DEBUG, (char *) &debugToggle,
				sizeof(debugToggle)) == -1) {
#ifdef WINDOWS_XP
			int errorCode;
...
void mySocket::setLingerSeconds(int seconds) {
	...
	try {
		if (setsockopt(socketId, SOL_SOCKET, SO_LINGER, (char *) &lingerOption,
...
void mySocket::setSendBufSize(int sendBufSize) {
	try {
		if (setsockopt(socketId, SOL_SOCKET, SO_SNDBUF, (char *) &sendBufSize,
...
int mySocket::getDebug() {
	...
	try {
		if (getsockopt(socketId, SOL_SOCKET, SO_DEBUG, (char *) &myOption,
				&myOptionLen) == -1) {
#ifdef WINDOWS_XP
			int errorCode;
...
int mySocket::getReuseAddr() {
	...
	try {
		if (getsockopt(socketId, SOL_SOCKET, SO_REUSEADDR, (char *) &myOption,
				&myOptionLen) == -1) {
#ifdef WINDOWS_XP
			int errorCode;
...
int mySocket::getKeepAlive() {
	...
	try {
		if (getsockopt(socketId, SOL_SOCKET, SO_KEEPALIVE, (char *) &myOption,
				&myOptionLen) == -1) {
#ifdef WINDOWS_XP
			int errorCode;
...
int mySocket::getLingerSeconds() {
	...
	try {
		if (getsockopt(socketId, SOL_SOCKET, SO_LINGER, (char *) &lingerOption,
				&myOptionLen) == -1) {
#ifdef WINDOWS_XP
			int errorCode;
...
int mySocket::getSendBufSize() {
	...
	try {
		if (getsockopt(socketId, SOL_SOCKET, SO_SNDBUF, (char *) &sendBuf,
				&myOptionLen) == -1) {
#ifdef WINDOWS_XP
			int errorCode;
...
int mySocket::getReceiveBufSize() {
	...
	try {
		if (getsockopt(socketId, SOL_SOCKET, SO_RCVBUF, (char *) &rcvBuf,
				&myOptionLen) == -1) {
#ifdef WINDOWS_XP
			int errorCode;
...
ostream& operator<<(ostream& io, mySocket& s) {
	...
	io << "   debug:         " << (flagStr = s.getDebug() ? "true" : "false")
			<< endl;
	io << "   reuse addr:    "
			<< (flagStr = s.getReuseAddr() ? "true" : "false") << endl;
	io << "   keep alive:    "
			<< (flagStr = s.getKeepAlive() ? "true" : "false") << endl;
	io << "   send buf size: " << s.getSendBufSize() << endl;
	io << "   recv bug size: " << s.getReceiveBufSize() << endl;
	io << "   blocking:      "
			<< (flagStr = s.getSocketBlocking() ? "true" : "false") << endl;
	io << "   linger on:     "
...
void myTcpSocket::initialize() {
	...
	try {
		if (WSAStartup(0x101, &wsaData)) {
			myException initializationException(0,
...
void myTcpSocket::detectErrorBind(int* errCode, string& errMsg) {
	*errCode = WSAGetLastError();
	...
	if (*errCode == WSANOTINITIALISED)
		...
		errMsg.append(
				"the user address space, the namelen parameter is too small,\n");
		errMsg.append(
				"the name parameter contains incorrect address format for the\n");
		errMsg.append(
				"associated address family, or the first two bytes of the memory\n");
		errMsg.append(
				"block specified by name does not match the address family\n");
...
void myTcpSocket::detectErrorRecv(int* errCode, string& errMsg) {
	*errCode = WSAGetLastError();
	...
	if (*errCode == WSANOTINITIALISED)
		...
		errMsg.append(
				"The connection has been broken due to the keep-alive activity\n");
		errMsg.append(
		...
		errMsg.append(
				"The socket has been shut down; it is not possible to recv on a\n");
		errMsg.append(
		...
		errMsg.append(
				"other failure. The application should close the socket as it is no longer usable.");
		...
		errMsg.append(
				"\"hard\" or \"abortive\" close. The application should close\n");
		errMsg.append(
				"the socket as it is no longer usable. On a UDP datagram socket\n");
		errMsg.append(
...
void myTcpSocket::detectErrorConnect(int* errCode, string& errMsg) {
	*errCode = WSAGetLastError();
	...
	if (*errCode == WSANOTINITIALISED)
		...
		errMsg.append(
				"was not marked to allow address reuse with SO_REUSEADDR. This\n");
		errMsg.append(
				"error usually occurs when executing bind, but could be delayed\n");
		errMsg.append(
				"until this function if the bind was to a partially wild-card\n");
		errMsg.append(
				"address (involving ADDR_ANY) and if a specific address needs\n");
		...
		errMsg.append(
				"the user address space, the namelen parameter is too small,\n");
		errMsg.append(
				"or the name parameter contains incorrect address format for\n");
...
void myTcpSocket::detectErrorListen(int* errCode, string& errMsg) {
	*errCode = WSAGetLastError();
	...
	if (*errCode == WSANOTINITIALISED)
		...
		errMsg.append(
				"The socket's local address is already in use and the socket was\n");
		errMsg.append(
				"not marked to allow address reuse with SO_REUSEADDR. This error\n");
		errMsg.append(
				"usually occurs during execution of the bind function, but could\n");
		errMsg.append(
				"be delayed until this function if the bind was to a partially\n");
		errMsg.append(
				"wild-card address (involving ADDR_ANY) and if a specific address\n");
		errMsg.append(
	...
	*errCode = WSAGetLastError();
	...
	if (*errCode == WSANOTINITIALISED)
		...
		errMsg.append(
				"The socket has not been bound with bind, or an unknown flag\n");
		errMsg.append(
				"was specified, or MSG_OOB was specified for a socket with SO_OOBINLINE enabled.");
	} else if (*errCode == WSAECONNABORTED) {
		errMsg.append(
				"The virtual circuit was terminated due to a time-out or \n");
		errMsg.append(
...
int myTcpSocket::sendMessage(string message) {
	...
	// Sends the message to the connected host
	try {
		if ((numBytes = send(socketId, sendMsg.c_str(), sendMsg.size(), 0))
				== -1) {
#ifdef WINDOWS_XP
			int errorCode = 0;
			string errorMsg = "error calling send():\n";
...
int myTcpSocket::XPreceiveMessage(string& message) {
	...
	try {
		while (received < msgSize) {
			...
			if (charMsg[received] == SOH){		// Start Of Header found
				msgSize= 0;
			}
...
int myTcpSocket::receiveMessage(string& message) {
	...
	try {
		...
		if (numBytes == -1) {
			myException unixSocketRecvException(0,
					"unix: error calling recv()");
